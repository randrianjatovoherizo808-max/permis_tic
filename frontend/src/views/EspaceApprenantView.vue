<template>
  <div class="apprenant-page">
    ...
    <!-- MODALE : Visualisation du certificat (lecture seule, rendu identique à l'impression) -->
    <div v-if="certificatOuvert" class="cert-modal-overlay" @click.self="fermerCertificat" @contextmenu.prevent>
      <div class="cert-modal-box">
        ...
        <div
          class="cert-iframe-wrap"
          :style="{ width: (1122 * certScale) + 'px', height: (794 * certScale) + 'px' }"
          @contextmenu.prevent
        >
          <iframe
            class="cert-iframe"
            :srcdoc="certificatHtml"
            :style="{ transform: 'scale(' + certScale + ')' }"
            sandbox=""
            title="Certificat (lecture seule)"
          ></iframe>
        </div>
...
// ✅ Génère EXACTEMENT le même HTML que l'aperçu "Imprimer" de l'administrateur
// (mêmes logos, mêmes styles, mêmes dimensions) — aucune divergence de rendu.
function genererCertificatHtml(c) {
  if (!c) return ''
...
function voirCertificat(c) {
  ...
  certificatHtml.value = genererCertificatHtml(c)
  // Le certificat est une page A4 paysage (297mm ≈ 1122px) : on calcule une
  // échelle pour qu'il tienne dans la modale, sans jamais altérer son contenu.
  const largeurDispo = Math.min(window.innerWidth * 0.9, 960) - 32
  certScale.value = Math.min(1, largeurDispo / 1122)
}

function fermerCertificat() {
  certificatOuvert.value = null
  certificatHtml.value = ''
}
...
/* Modale de visualisation du certificat (lecture seule, rendu identique à l'impression) */
...
.cert-view-watermark { margin-top: 12px; text-align: center; font-size: 0.72rem; color: var(--gray); font-style: italic; }
